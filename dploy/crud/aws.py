...
def get_keys(db: Session, user_id):
    try:
        access_keys = db.query(AWS_Keys).filter(
            AWS_Keys.user_id == user_id).first()
...
def get_images(db: Session, user_id, image_ids):
    ...
    ec2_client = create_client(
        access_keys.access_key, access_keys.secret_access_key, "ec2")

    try:
        images = ec2_client.describe_images(ImageIds=image_ids)
    except ClientError as err:
        logging.error("Couldn't get images. Here's why: %s: %s",
...
def get_appropiate_instances(db: Session, user_id, image_id):
    ...
    try:
        ...
        logging.error("Couldn't Find Appropiate Instances. Here's why: %s: %s",
                      err.response['Error']['Code'], err.response['Error']['Message'])
        raise
    except Exception as err:
        logging.error(err)
...
def create_instances(db: Session, user_id, image_id, instance_type, key_name):
    ...
    try:
        ...
        db.add(new_instance)
        new_instance_key_pair = AWS_Instance_Keys(
            new_instance.id, key_pair["KeyName"], key_pair["KeyMaterial"])
        db.add(new_instance_key_pair)
        # Store the name,private key with user id and instance id in a separate table
    except ClientError as err:
        logging.error("Couldn't create instance with image %s, instance type %s, and key %s. "
...
def get_images_details(db: Session, user_id):
    ...
    try:
        ssm_client = create_client(
            access_keys.access_key, access_keys.secret_access_key, "ssm")
        ami_options = []
...
def create_key_pair(db: Session, user_id, key_name):
    ...
    try:
        ...
        logging.error("Couldn't Create Key Pair. Here's why: %s: %s",
                      err.response["Error"]['Code'], err.response['Error']['Message'])
    else:
        return key_pair
